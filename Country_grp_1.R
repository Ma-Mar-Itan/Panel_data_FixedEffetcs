...
summary(ols)
##Fixed model 
fe <- plm(ln_trade ~ ln_cti + ln_gdpc + distance + crises, data = df1, index = c("country", "year"), model = "within")
summary(fe)
##Random effect
...
vif_model <- lm(ln_trade ~ ln_cti + ln_gdpc + distance + crises, data = df1)
vif(vif_model)
